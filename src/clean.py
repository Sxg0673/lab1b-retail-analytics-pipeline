...
def clean_sales(df):  # Limpia y normaliza los datos de ventas, devolviendo un DataFrame limpio y otro con registros rechazados.
    ...
    # 3. Normalizar promotion_code: quitar espacios, unificar formato de texto y reemplazar placeholders por NaN.
    df["promotion_code"] = df["promotion_code"].replace("", np.nan) # Reemplazar cadenas vacías por NaN
    es_placeholder_faltante = df["promotion_code"].astype(str).str.strip().str.upper().isin(["N/A", "NA", "NAN"]) # Identificar placeholders de faltante
    df["promotion_code"] = df["promotion_code"].where(~es_placeholder_faltante, np.nan) # Reemplazar placeholders por NaN
